...
void initialize_vs() {
  ...
  for (int i = 0; i < 100; i++) {
    for (int j = 0; j < 200; j++) {
      vs[i * 100 + j] =
          Vec3(std::sin(polarangles[i]) * std::cos(azimuthalangles[j]),
...
void acceptanceRate(double *acceptance_ptr, double *dpotentials_ptr,
                    const System<BOX_N, NUM_ATOMS> &atom_system,
                    const double &sum_of_potentials, const int &atom_idx,
                    const Vec3 &new_position, const double &T) {
  const double system_size = atom_system.getSystemSize();
  const std::array<double, NUM_ATOMS> &potentials =
      atom_system.getPotentialEnergies();
  ...
  for (const size_t i : atom_system.getAdjacentAtoms(atom_idx)) {
    if (i != atom_idx) {
      Vec3 position = atom_system.getAtom(i).getPosition();
      Vec3 diff_old = PeriodicDifference(
          atom_system.getAtom(atom_idx).getPosition(), position, system_size);
...
void MC_step(System<BOX_N, NUM_ATOMS> *atom_system_ptr,
             double *sum_of_potentials_ptr, const int &atom_idx,
  ...
  const Vec3 displacement =
      dr / std::sqrt(3) * Vec3(2 * rd() - 1, 2 * rd() - 1, 2 * rd() - 1);
  const Vec3 prop_position =
      atom_system.PeriodicPositionUpdate(position, displacement, 1.0);
  double acceptance_rate;
  auto *dpotentials = new double[N];
  memset(dpotentials, 0, N * sizeof(double));
  acceptanceRate(&acceptance_rate, dpotentials, atom_system, sum_of_potentials,
                 atom_idx, prop_position, T);
...
void MC_sweep(System<BOX_N, NUM_ATOMS> *atom_system_ptr,
              double *sum_of_potentials_ptr, UniformRandomFloat *rd_ptr,
              const double &dr, const double &T, int *Naccept_ptr) {
  auto &atom_system = *atom_system_ptr;
  constexpr size_t N = NUM_ATOMS;
  for (int i = 0; i < N; i++) {
...
int main(int argc, char *argv[]) {
  ...
  initialize_vs();
  System<BOX_N, NUM_ATOMS> atom_system(system_size, positions, velocities,
                                       T_init);
  // start the timer
  ...
  for (size_t i = 0; i < sweeps; i++) {
    ...
    if (i % (sweeps / 50) == 0) {
      ...
      std::cout << "] " << 2 * (i * 50 / sweeps) << " % "
                << static_cast<double>(global_Naccept) / (N * (i + 1)) << " "
                << dr << "\r";
      std::cout.flush();
    }
