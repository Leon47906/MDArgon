...
const static std::array<Vec3,6> unit_velocities{Vec3(1,0,0), Vec3(0,1,0), Vec3(0,0,1), Vec3(-1,0,0), Vec3(0,-1,0), Vec3(0,0,-1)};
...
class System{
    ...
    System(const double _system_size, const std::vector<Vec3> &_positions,
             const std::vector<Vec3> &_velocities, double _T_init) : system_size(_system_size),
      T_init(_T_init), cells(box_N*box_N*box_N) {
        ...
        for (size_t i = 0; i < N; i++) {
            ...
            index += static_cast<size_t>(std::floor(position.y / box_L))
            * box_N;
            index += static_cast<size_t>(std::floor(position.z / box_L))
            * box_N * box_N;
    ...
    */
    ...
    [[nodiscard]] auto getPotentialEnergies() const {
        return E_pot;
    }
    ...
    [[nodiscard]] size_t getCellIdx(const Vec3 &position) const {
        ...
        index += static_cast<size_t>(std::floor(position.z / box_L)) * box_N
        * box_N;
        return index;
    }
    ...
    [[nodiscard]] static std::array<size_t,26> getNeighboringCells(const size_t
        cell_index) {
            ...
            for (int j = -1; j <= 1; j++) {
                for (int k = -1; k <= 1; k++) {
                    ...
                    if (const size_t neighbor_index =
                            x_neighbor + y_neighbor * box_N +
                            z_neighbor * box_N * box_N;
    ...
    [[nodiscard]] auto getCell(const size_t cell_index)
    const {
        return cells[cell_index];
    }
    // Funktion, welche die Indizes der Atome in den Nachbarzellen einer Zelle zurückgibt
    [[nodiscard]] std::array<size_t,N> getAtomsInNeighboringCells(const
    ...
    [[nodiscard]] std::array<size_t, N> getAdjacentAtoms(const
        ...
        const std::array<size_t, N> neighbors = getAtomsInNeighboringCells(cell_index);
    ...
    [[nodiscard]] std::array<size_t, N> getAdjacentAtoms(const Vec3& position)
    const {
    ...
    [[nodiscard]] Vec3 PeriodicPositionUpdate(const Vec3& position, const Vec3& velocity, const double& dt) const {
    ...
    void computeAccels() {
        ...
        for (size_t cell_index = 0; cell_index < box_N * box_N * box_N; ++cell_index) {
            ...
            for (const size_t neighbor_cell_idx : neighboring_cells) {
                ...
                for (const size_t atom_i : cell) {
                    for (const size_t atom_j : cell1) {
                        const Vec3 r = PeriodicDifference(atoms[atom_i].getPosition(), atoms[atom_j].getPosition(),system_size);
                        const double r2 = r.norm2();
                        const Vec3 accel = r * ComputeAccel(r2);
                        const double pot = LennardJones(r2);
                        accels[atom_i] += accel;
    ...
    double computePotentialEnergy() {
        ...
        for (size_t i = 0; i < N; i++) {
            for (size_t j = i + 1; j < N; j++) {
                const Vec3 r = PeriodicDifference(atoms[i].getPosition(), atoms[j].getPosition(), system_size);
                const double r2 = r.norm2();
                const double pot = LennardJones(r2);
                E_pot[i] += pot/2;
